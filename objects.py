...
class Accretion:
    ...
    def UVExcessErrorProp(self, SpTyUnc, distUnc, ageUnc, AvUnc, bc, bcUnc, UVExcessUnc, numMC, Rin=5):
            ...
            #ageUncDist = a.uncdist(x, self.age, ageUnc, numMC)
    ...
    def linefluxErrorProp(self, SpTyUnc, distUnc, ageUnc, AvUnc, linefluxUnc, numMC, Rin=5, line=None, A=None, AUnc=None, B=None, BUnc=None):
            #propagate errors forward and obtain uncertainty distributions
            if line == None:
                A = A
                aUnc = AUnc
...
##### This object represents a distribution of accreting objects whose intrinsic properties are derived from 
##### PDF fitted priors of mass, age, distance, and extinction.
##### Because the values are drawn randomly from these priors for each individual object, they don't necessarily represent
##### the current observed population which is clustered around specific star forming regions (e.g. Taurus).
...
class AccretionDistribution:
    ...
    def UVExcessErrorProp(self, SpTyUnc, distUnc, ageUnc, AvUnc, bc, bcUnc, UVExcessUnc, numMC, Rin=5):
        for acc in self.accretion_distribution:
            acc.UVExcessErrorProp(SpTyUnc, distUnc, ageUnc, AvUnc, bc, bcUnc, UVExcessUnc, numMC, Rin=Rin)
    
    def linefluxErrorProp(self, SpTyUnc, distUnc, ageUnc, AvUnc, linefluxUnc, numMC, Rin=5, line=None, A=None, AUnc=None, B=None, BUnc=None):
        for acc in self.accretion_distribution:
            acc.linefluxErrorProp(SpTyUnc, distUnc, ageUnc, AvUnc, linefluxUnc, numMC, Rin=5, line=line, A=A, AUnc=AUnc, B=B, BUnc=BUnc)
...
def MarginalDistribution(observed, simulated):
    ...
    #plot
    ax2_hist.hist(observed_difference, orientation='horizontal', color='black', density=True, alpha=0.225, label='Observed')
    ax2_hist.hist(simulated_difference, orientation='horizontal', color='darkseagreen', density=True, alpha=0.3, label='Simulated')
...
def residuals(observed, simulated):
    ...
    rect_ax2_hist = [left+width+spacing, bottom, 0.2, height]
